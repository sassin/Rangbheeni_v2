...
function getInitialAnnouncement(
  initialAnnouncement: LaunchAnnouncement | null
): LaunchAnnouncement {
  return initialAnnouncement?.id ? initialAnnouncement : defaultAnnouncement;
}
...
export function AnnouncementClient({
  ...
  const selectedAnnouncement = useMemo(
    () => getInitialAnnouncement(initialAnnouncement),
    [initialAnnouncement]
  );

  const [announcement, setAnnouncement] = useState<LaunchAnnouncement | null>(() => {
    if (!enabled || typeof window === "undefined") return null;

    const key = `rangbheeni_announcement_dismissed_${selectedAnnouncement.id}`;
